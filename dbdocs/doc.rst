...
github page (https://github.com/usnistgov/jarvis). We use the projected
augmented wave method .  Both the internal atomic positions and the
lattice constants are allowed to relax in spin-unrestricted calculations
until the maximal residual Hellmann–Feynman forces on atoms are smaller
than 0.001 eV Å\ :sup:`−1` and energy-tolerance of 10\ :sup:`-7` eV with
accurate precision setting (PREC=Accurate). Note that force convergence
is very important for properties such as phonons, elastic constants etc.
We use spin-polarized set-up with initial magnetic moment of default
value 1 muB during geometric optimization. Also use conjugate gradient
algorithm for ionic relaxation (IBRION=2).
...
where :math:`E_{x}^{\text{GGA}}` is the exchange energy within the
generalized gradient approximation (GGA) and :math:`E_{c}^{\text{LDA}}`
is the correlation energy within the local-density approximation (LDA).
The nonlocal correlation energy is given by:

:math:`E_{c}^{\text{NL}} = \frac{1}{2}\iint_{}^{}{d\mathbf{r}d\mathbf{r}^{\mathbf{'}}n\left( \mathbf{r} \right)ф\left( d,d^{'} \right)n\left( \mathbf{r}^{'} \right)}`

where :math:`ф` is a kernel function,
:math:`d = q_{0}\left( r \right)\left| r - r^{'} \right|` and
:math:`d' = q_{0}\left( r' \right)\left| r - r^{'} \right|`. The
q\ :sub:`0` is a function of n(r) and \|∇n(r)|, and it is proportional
to the gradient corrected LDA exchange-correlation energy per electron.
This function controls the behavior of :math:`E_{c}^{\text{NL}}` in the
slowly varying as well as nonuniform density regions. It is noted that
the use of the LDA correlation is motivated by the fact that
:math:`E_{c}^{\text{NL}}`\ vanishes in the uniform electron gas limit,
and to avoid the possible double counting of the gradient correction
contained in :math:`E_{c}^{\text{NL}}`. Hence, the vdW-DF-optB88 is an
example of the truly nonlocal-correlation functionals in the
vdW-DF-method for approximating the vdW forces in regular DFT.
...
With , Å\ :sup:`1/2` and V\ :sub:`cell` is the volume of the unit cell.
The *c*-parameter was automatically determined in VASP through a
self-consistent run.
...
:math:`\eta\left( \mathbf{k} \right),` given by the following equation:

:math:`\eta\left( \mathbf{k} \right) = n_{\text{occ}}(\mathbf{k}) - \text{Tr}\left( P\widetilde{P} \right)`

where

:math:`P\left( \mathbf{k} \right) = \sum_{n = 1}^{n_{\text{occ}}(\mathbf{k})}{\left. \ |\psi_{n\mathbf{k}} \right\rangle\left\langle \psi_{n\mathbf{k}} \right.\ |}`
for wavefunctions without SOC and :math:`\widetilde{P}\ `\ with SOC
calculations.
...
correlation energy. In HSE (Heyd–Scuseria–Ernzerhof):sup:`32`, the
exchange is given by a screened Coulomb potential, to improve
computational efficiency. An adjustable parameter (ω) controls how short
range the interaction is. HSE06 is characterized by ω=0.2, while for a
choice of ω=0 HSE becomes PBE0.
...
insulating system with *N* atoms per cell, with cell volume Ω\ :sub:`0`,
atomic displacements u\ :sub:`m` (m=(1…3N)), homogenous strain
η\ :sub:`j` (j=(1…6)), homogenous electric fields
:math:`\mathcal{\text{\ E}}`\ :sub:`α` (α=(x,y,z)), energy *E*, the
force-constant matrix (:math:`K_{\text{mn}}`), internal strain tensor
(:math:`\Lambda_{\text{mj}}`), the dielectric susceptibility
(:math:`\chi_{\text{αβ}}`), Born dynamical effective charge
(:math:`Z_{\text{mα}}`), piezoelectric stress tensor
(:math:`e_{\text{αj}}`) and piezoelectric strain tensor
(:math:`d_{\text{αj}}`) are calculated as follows (SI units are used
throughout)\ :sup:`65-69`:

:math:`K_{\text{mn}} = \Omega_{0}\frac{\partial^{2}E}{\partial u_{m}\partial u_{n}}|_{\mathcal{E,}\eta}`

:math:`Z_{\text{mα}} = - \Omega_{0}\frac{\partial^{2}E}{\partial u_{m}\partial\mathcal{E}_{\alpha}}|_{\eta}`

:math:`\Lambda_{\text{mj}} = - \Omega_{0}\frac{\partial^{2}E}{\partial u_{m}\partial\eta_{j}}|_{\mathcal{E}}`

:math:`\chi_{\text{αβ}} = = - \frac{\partial^{2}E}{\partial\mathcal{E}_{\alpha}\partial\mathcal{E}_{\beta}}|_{u,\eta} + \Omega_{0}^{- 1}Z_{\text{mα}}\left( K^{- 1} \right)_{\text{mn}}Z_{\text{nβ}}`

:math:`e_{\text{αj}} = - \frac{\partial^{2}E}{\partial\mathcal{E}_{\alpha}\partial\eta_{j}}|_{u} + \Omega_{0}^{- 1}Z_{\text{mα}}\left( K^{- 1} \right)_{\text{mn}}\Lambda_{\text{nj}}`

:math:`d_{\text{αj}} = S_{\text{jk}}^{\left( \mathcal{E} \right)}e_{\text{αk}}`

The dielectric constant can be derived from the dielectric
susceptibility using:

:math:`{\varepsilon_{\text{αβ}} = \varepsilon_{0}(\delta_{\text{αβ}} + \chi}_{\text{αβ}})`
...
:math:`f\left( n \right) = \sum_{\alpha}^{}\left| \sum_{\text{sβ}}^{}Z_{\text{αβ}}\left( s \right)e_{\beta}\left( s,n \right) \right|^{2}`

where :math:`e_{\beta}\left( s,n \right)` is the normalized vibrational
eigenvector of the \ *n*\ th phonon mode of the \ *s*\ :sup:`th` atom in
the unit cell, and α, β are the cartesian coordinates.
:math:`Z_{\text{αβ}}\left( s \right)` is the Born effective charge
tensor of *s*\ :sup:`th` atom (here we explicitly write both the
cartesian indices of Z). These approaches are universal and have been
already applied to various material classes. More details about the DFPT
formalism can be found in elsewhere\ :sup:`65,66`.
...
relationship. A set of strains **ε** where ε\ :sub:`1`, ε\ :sub:`2`, and
ε\ :sub:`3` are the normal strains and the others are the shear strains
imposed on a crystal with lattice vectors **R** specified in Cartesian
coordinates,
...
| :math:`\varepsilon_{\text{αβ}}^{(2)}\left( E \right) = \frac{4\pi^{2}e^{2}}{\Omega^{2}}\operatorname{}{\frac{1}{q^{2}}\sum_{c,v,\overrightarrow{k}}^{}{2w_{\overrightarrow{k}}\delta\left( \xi_{c\overrightarrow{k}} - \xi_{v\overrightarrow{k}} - E \right)\left\langle \Psi_{c\overrightarrow{k} + {\overrightarrow{e}}_{\alpha}q}|\Psi_{v\overrightarrow{k}} \right\rangle\left\langle \Psi_{v\overrightarrow{k}}|\Psi_{c\overrightarrow{k} + {\overrightarrow{e}}_{\beta}q} \right\rangle^{*}}}`
| where *e* is electron charge, :math:`\Omega` is the cell volume, *E*
  the energy, |image2|\ is the Fermi-weight of each *k*-point,
  |image3|\ are unit vectors along the three Cartesian directions, is
  the cell-periodic part of the pseudo-wavefunction for band *n* and
  k-point :math:`k`, *q* stands for the Bloch vector of an incident
  wave, *c* and *v* stand for conduction and valence bands, stands for
  eigenvalues of the corresponding bands respectively. The matrix
  elements on the right side of Eq. (1) capture the transitions allowed
  by symmetry and selection rules. The real part of the dielectric
  tensor \ :math:`\varepsilon_{\text{αβ}}^{(1)}` is obtained by the
  usual Kramers-Kronig transformation:

:math:`\varepsilon_{\text{αβ}}^{(1)}\left( E \right) = 1 + \frac{2}{\pi}P\int_{0}^{\infty}\frac{\varepsilon_{\text{αβ}}^{(2)}\left( E^{'} \right)E^{'}}{\left( E^{'} \right)^{2} - E^{2} + i\eta}dE^{'}`
...
dielectric function (written as :math:`\varepsilon^{(1)}\ `\ and
:math:`\varepsilon^{(2)}`), obtained by diagonalizing the dielectric
tensor for each energy and averaging the diagonal elements.

Using, :math:`\varepsilon^{(1)}` and :math:`\varepsilon^{(2)}` the
absorption coefficient :math:`\alpha\left( E \right)` is defined as:

:math:`\alpha\left( E \right) = \frac{2E}{\text{ℏc}}\sqrt{\frac{\sqrt{\left( \varepsilon^{(1)}\left( E \right) \right)^{2} + \left( \varepsilon^{(2)}\left( E \right) \right)^{2}} - \left( \varepsilon^{(1)}\left( E \right) \right)}{2}}`

where *c* is the speed of light.

Next, the SLME (:math:`ɳ)` is defined as the ratio of the maximum output
power density (*P\ max*) and the total incident solar energy density
(*P\ in*). *P\ max* is obtained by numerically maximizing the product of
current density *J* and voltage *V*.

:math:`ɳ = \frac{P_{\max}}{P_{\text{in}}}`
...
:math:`J = J_{\text{sc}} - J_{0}\left( e^{\frac{\text{eV}}{\text{kT}}} - 1 \right)`
...
:math:`J_{\text{sc}} = e\int_{0}^{\infty}{a\left( E \right)I_{\text{sun}}\left( E \right)\text{dE}}`

where and *a(E)* is the photon absorptivity,
:math:`I_{\text{sun}}\ `\ is the AM1.5G solar spectrum. The *a(E)*
depends on the absorption coefficient (:math:`\alpha`) (Eq. (3)) and
thickness (*L*) of the material.

:math:`a\left( E \right) = 1 - e^{- 2\alpha\left( E \right)L}`
...
:math:`J_{0}^{} = J_{0}^{r} + J_{0}^{\text{nr}} = \frac{J_{0}^{r}}{f_{r}}`

Here, *f\ r* is defined as the fraction of the radiative recombination
current. For the SLME, *f\ r* is approximated using:

:math:`f_{r} = e^{\left( \frac{E_{g} - E_{g}^{\text{da}}}{\text{kT}} \right)}`

Where :math:`E_{g}` is the fundamental and
:math:`E_{g}^{\text{da}}\ `\ is the direct allowed bandgap of a
material.
...
dark. Hence, :math:`J_{0}^{r}` can be calculated from the rate at which
black-body photons from the surrounding thermal bath are absorbed
through the front surface, given by:

:math:`J_{0}^{r} = e\pi\int_{0}^{\infty}{a\left( E \right)I_{\text{bb}}\left( E,T \right)\text{dE}}`

where *I\ bb* is the black-body spectrum at temperature *T.* Both the
solar spectrum :math:`I_{\text{sun}}\ `\ and black-body spectrum
:math:`I_{\text{bb}}` are expressed in terms of the photon flux.

In order to maximize the power density, Eq. can be re-written as:

:math:`ɳ = \frac{P_{\max}}{P_{\text{in}}} = \frac{\max\left\{ \left( J_{\text{sc}} - J_{0}\left( e^{\frac{\text{eV}}{\text{kT}}} - 1 \right) \right)V \right\}_{V}}{\int_{0}^{\infty}{EI_{\text{sun}}\left( E \right)\text{dE}}}`

Therefore, the material-property related inputs in calculating the SLME
are :math:`\alpha\left( E \right),f_{r},\ L` and T. In this work, we
assume material thickness (*L*) as 500 nm and temperature (*T*) as 300
K.
...
readers can see longer discussions in :sup:`5,16`. For a set of Bloch
eigenvectors :math:`|\left. \ \psi_{n,k} \right\rangle`, a general set
of WFs :math:`\left. \ |\mathbf{R}n \right\rangle` (n=1...N) can be
written as:

:math:`\left. \ |\mathbf{R}n \right\rangle = \frac{V}{\left( 2\pi \right)^{3}}\int_{\text{BZ}}^{\text{BZ}}{\sum_{m = 1}^{N}U_{\text{mn}}^{(\mathbf{k})}|\left. \ \psi_{\text{mk}} \right\rangle e^{- i\mathbf{k}.\mathbf{R}}d\mathbf{k}}`

where\ :math:`\ \mathbf{R}` **labels the unit cell of the WF, V is the
volume of the unit cell, and** :math:`U_{\text{mn}}^{(\mathbf{k})}` is
an arbitrary unitary matrix. To construct maximally-localized WFs,
:math:`U_{\text{mn}}^{(\mathbf{k})}` is chosen to minimize the following
spread functional:

:math:`\Omega = \sum_{n}^{}\left\lbrack \left\langle r^{2} \right\rangle_{n} - {\overline{r}}_{n}^{2} \right\rbrack`

where
:math:`{\overline{\mathbf{r}}}_{n} = \ \left\langle 0n \middle| \mathbf{r} \middle| 0n \right\rangle`
and
:math:`\left\langle r^{2} \right\rangle_{n} = \ \left\langle 0n \middle| r^{2} \middle| 0n \right\rangle`.
...
Wannierization. We use a frozen window of :math:`\pm \ `\ 2 eV around
the Fermi-energy. This window ensures that bands near the Fermi level
are well described by the WTBH. For cases where the original WFs were
...
validate our WTBH, we calculate the maximum absolute difference
(:math:`\mu`) between the Wannier and DFT eigenvalues within an energy
range of ± 2eV around the Fermi level:

:math:`\mu =_{n\mathbf{k}}^{\max}{\ \left( \left| E_{n\mathbf{k}}^{\text{DFT}} - E_{n\mathbf{k}}^{\text{WTB}} \right| \right)}`
...
:math:`\frac{dE_{\text{nk}}}{\text{dk}}`) can result in high :math:`\mu`
values even if the WTBH is of good quality because any slight shift in
the *k*-direction of a dispersive band will result in a large energy
error. We consider that systems with :math:`\mu` less than 0.1 eV to
useful for most applications, and we provide data for the user to
evaluate individual WTBH for their own applications.
...
:math:`\frac{\text{df}\left( \mathbf{k},T,t \right)}{\text{dt}} = \left( \frac{\partial f\left( \mathbf{k},T,t \right)}{\partial t} \right)_{s} - \frac{d\mathbf{k}}{\text{dt}}\nabla_{k}f\left( \mathbf{k},T,t \right) - v\left( \mathbf{k} \right)\nabla_{r}f\left( \mathbf{k},T,t \right)`.
...
B, such that :math:`\frac{\text{dk}}{\text{dt}} = \frac{\text{eE}}{h}`
then the low-filed BTE is given by:

:math:`\frac{\text{df}\left( \mathbf{k},T,t \right)}{\text{dt}} + v\left( \mathbf{k} \right)\nabla_{r}f\left( \mathbf{k},T \right) + \frac{\text{eE}}{h}\nabla_{k}f\left( \mathbf{k},T \right) = \left( \frac{\partial f\left( \mathbf{k},T,t \right)}{\partial t} \right)_{s}`.
...
:math:`\left( \frac{\partial f\left( \mathbf{k},T,t \right)}{\partial t} \right)_{s} = - \frac{f\left( k \right) - f_{0}\left( k \right)}{\tau}`
...
where the dependence of :math:`\mathcal{E}` on *k* is given by the
electronic band structure, and the various

scattering terms and time dependence are lumped into the electronic
relaxation time,\ :math:`\text{\ τ}`. The computation of the relaxation
time is very computationally expensive, especially in a high-throughput
context.
...
Fermi energy (E:sub:`F`) and valence band maxima (VBM) to Fermi energy
(E:sub:`F`). The STM images are calculated using Tersoff-Hamann
approach:

:math:`I_{\mu} \propto \rho\left( r,E_{F} \right) \equiv \sum_{\mu}^{}{\left| \psi_{\mu}\left( r \right) \right|^{2}\delta\left( \varepsilon_{\mu} - E_{F} \right)}`

where :math:`I_{\mu}` is the tunneling current, :math:`\psi_{\mu}` the
eigenvector and :math:`\varepsilon_{\mu}` the eigenenergy due to the
state μ and :math:`E_{F}` is the Fermi-energy.
...
obtained from*
`18 <#_ENREF_18>`__\ *\ \ \ *\ `21 <#_ENREF_21>`__\ *\ \ \ *\ `46 <#_ENREF_46>`__\ *\ \ \ *\ `47 <#_ENREF_47>`__\ *\ ,,,.
...
BN                        P6\ :sub:`3`/mmc 167799 984   17   4.48 4.51  4.4  4.46 6.11 5.5   6.2
AlN                       P6\ :sub:`3`\ mc 31169  661   39   4.06 4.06  4.5  4.47 5.20 5.49  6.19
AlN                       F-43m            67780  1700  7844 3.31 3.31  -    3.55 4.80 4.55  4.9
GaN                       P6\ :sub:`3`\ mc 34476  804   30   1.74 1.91  2.1  1.94 3.08 3.15  3.5
GaN                       F-43m            157511 830   8169 1.57 1.75  -    1.79 2.9  2.85  3.28
InN                       P6\ :sub:`3`\ mc 162684 22205 1180 0.0  0.0   -    0.23 0.76 -     0.72
...
MoS\ :sub:`2`             P6\ :sub:`3`/mmc 24000  2815  54   1.23 1.25  1.3  0.92 1.33 1.49  1.29
MoSe\ :sub:`2`            P6\ :sub:`3`/mmc 49800  1634  57   1.42 1.03  1.0  0.91 1.32 1.40  1.11
WS\ :sub:`2`              P6\ :sub:`3`/mmc 56014  224   72   1.56 1.29  1.4  0.72 1.51 1.6   1.38
WSe\ :sub:`2`             P6\ :sub:`3`/mmc 40752  1821  75   1.45 1.22  1.2  1.05 1.44 1.52  1.23
Al\ :sub:`2`\ O\ :sub:`3` R-3c             600672 1143  32   5.85 5.85  6.3  6.43 7.57 8.34  8.8
...
TiO\ :sub:`2`             P4\ :sub:`2`/mnm 9161   2657  5    1.78 2.26  1.8  1.77 2.07 3.34  3.30
TiO\ :sub:`2`             I4\ :sub:`1`/amd 9852   390   104  2.05 2.53  2.0  2.02 2.47 -     3.4
Cu\ :sub:`2`\ O           Pn-3m            26183  361   1216 0.5  -     0.8  0.13 0.49 1.98  2.17
CuAlO\ :sub:`2`           R-3m             25593  3748  1453 1.8  2.0   2.4  2.06 2.06 -     3.0
ZrO\ :sub:`2`             P2\ :sub:`1`/c   15983  2858  113  3.47 3.56  4.0  3.62 4.21 -     5.5
HfO\ :sub:`2`             P2\ :sub:`1`/c   27313  352   9147 4.02 4.02  4.5  4.12 5.66 -     5.7
CuCl                      F-43m            23988  22914 1201 0.56 1.28  0.8  0.45 1.59 2.37  3.4
SrTiO\ :sub:`3`           Pm-3m            23076  5229  8082 2.1  2.29  1.8  1.81 2.30 -     3.3
...
MAE\ :sup:`1`             -                -      -     -    1.45 1.23  1.14 1.33 0.51 0.41  -
MAE\ :sup:`2`             -                -      -     -    1.39 1.19  1.09 1.27 0.43 0.42  -
...
example of SOC spillage and band-inversion for
Bi\ :sub:`2`\ Se\ :sub:`3` is shown below:
...
|C:\Users\knc6\Downloads\mbj_op3.png|

*Fig. Imaginary part of the dielectric function in the x-direction
obtained from OPT, MBJ and experiments. a) 1T-SnSe\ 2
(*\ :math:`P\overline{3}m1`\ *), b) 2H-MoS\ 2 (P63/mmc), c) Si
(*\ :math:`\text{Fd}\overline{3}m`\ *), d) Ge
(*\ :math:`\text{Fd}\overline{3}m`\ *), e) GaAs
(*\ :math:`F\overline{4}3m)` *and f) InP* :math:`(F\overline{4}3m)` *.*
...
========================= ============= ================
======================
**Materials**     **JID** **E\ g (TBmBJ)** **E\ g (G0\ W\ 0)** **E\ g (G0\ W\ 0\ +SOC)** **Ƞ (TBmBJ)** **Ƞ (G0\ W\ 0)** **Ƞ (G0\ W\ 0\ +SOC)**
================= ======= ================ ===================
========================= ============= ================
======================
...
=============== ==== ====================== ======================
======================
MoS\ :sub:`2`   54   *ε*\ :sub:`11` =15.56  *ε*\ :sub:`11`\ =15.34 *ε*\ :sub:`11`\ =17.0
MoSe\ :sub:`2`  57   *ε*\ :sub:`11`\ =16.90 *ε*\ :sub:`11`\ =16.53 *ε*\ :sub:`11`\ =18.0
MoTe\ :sub:`2`  60   *ε*\ :sub:`11`\ =21.72 *ε*\ :sub:`11`\ =18.74 *ε*\ :sub:`11`\ =20.0
WS\ :sub:`2`    72   *ε*\ :sub:`11`\ =13.91 *ε*\ :sub:`11`\ =13.95 *ε*\ :sub:`11`\ =11.5
WSe\ :sub:`2`   75   *ε*\ :sub:`11`\ =15.21 *ε*\ :sub:`11`\ =14.32 *ε*\ :sub:`11`\ =11.7
SiC             182  7.10                   6.01                   6.552
AlP             1327 10.33                  6.94                   7.54
BN              17   *ε*\ :sub:`11`\ =4.75  *ε*\ :sub:`11`\ =3.72  *ε*\ :sub:`11`\ =5.06
BP              1312 9.03                   7.94                   11.0
GaP             1393 13.22                  8.33                   11.11
AlSb            1408 12.27                  9.87                   12.04
ZnS             1702 9.39                   4.8                    8.0
CdTe            23   19.59                  6.54                   10.6
HgTe            8041 *ε*\ :sub:`11`\ =29.44 *ε*\ :sub:`11`\ =11.22 *ε*\ :sub:`11`\ =20
ZnSiP\ :sub:`2` 2376 *ε*\ :sub:`11`\ =12.44 *ε*\ :sub:`11`\ =8.56  *ε*\ :sub:`11`\ =11.15
ZnGeP\ :sub:`2` 2355 *ε*\ :sub:`11`\ =14.75 *ε*\ :sub:`11`\ =9.02  *ε*\ :sub:`11`\ =15
MAE             -    2.46                   2.78                   -
=============== ==== ====================== ======================
======================
...
:math:`e_{\text{ij}}` or constant stress, giving the piezoelectric
strain tensor :math:`d_{\text{ij}}` . The PZ data is obtained using DFPT
method discussed above. Piezoelectric tensor is a 3x3 matrix. A
distribution of the PZ tensor components is shown below:
...
=============== ===== ================= ====== =========================
Mats.           JID   Max(e\ :sub:`ij`) DFT    Reference
=============== ===== ================= ====== =========================
BN              57695 1.55              1.15   :sup:`9`
AlN             39    1.39-1.55         1.39   :sup:`10-12`
ZnS             7648  0.38              0.13   :sup:`9`
ZnSe            8047  0.35              0.06   :sup:`9`
SiO\ :sub:`2`   41    0.171             0.16   :sup:`13,14`
BaTiO\ :sub:`3` 110   1.94-5.48         4.13   :sup:`15,16` :sup:`17,18`
LiNbO\ :sub:`3` 1240  1.33              1.59   :sup:`19`
GaSb            35711 -0.07             -0.102 :sup:`9`
PbTiO\ :sub:`3` 3450  3.35-5.0          3.96   :sup:`20`
GaN             30    0.73              0.47   :sup:`21`
InN             1180  0.97              0.90   :sup:`22`
AlP             1327  -0.1              0.004  :sup:`9`
AlAs            1372  -0.16             0      :sup:`9`
AlSb            1408  -0.13             0.06   :sup:`9`
ZnO             1195  1.00-1.55,0.89    1.10   :sup:`23`
BeO             20778 0.1               0.22   :sup:`23`
MAD                                     0.21  
=============== ===== ================= ====== =========================
...
classified in three categories: far (30-400 cm\ :sup:`-1`), mid
(400-4000 cm\ :sup:`-1`) and near (4000-14000 cm\ :sup:`-1`) IR
frequencies. The IR intensity is calculated obtained from the
gamma-point phonon data used in the DFPT calculations.
...
====== ===== =================================
===============================
ZnO    1195  379, 410                          389,413\ :sup:`1`
AlN    39    600, 653                          620,669\ :sup:`2`
GaN    30    532                               531\ :sup:`3,4`
SnS    1109  93, 144, 178, 214                 99, 145, 178, 220 :sup:`5`
SnSe   299   72.6, 98.44, 125.01, 160.3        80,96,123, 150 :sup:`5`
KMgF3  20882 160.0, 287.0, 470.8               168, 299, 458\ :sup:`6`
LiNbO3 1240  145.0, 216.6                      160, 220\ :sup:`7`
GeS    2169  106.4, 196.2, 236.5, 253.0, 276.9 118, 201,238,258, 280\ :sup:`8`
MAD                                            8.36
====== ===== =================================
===============================
...
:math:`{s_{\text{ij}} = C}_{\text{ij}}^{- 1}`

Now, several other elastic properties calculated from C\ :sub:`ij` and
s\ :sub:`ij`. Some of the important properties are given below:

K\ :sub:`V` = ((C:sub:`11`\ +C\ :sub:`22`\ +C\ :sub:`33`) +
2(C\ :sub:`12`\ +C\ :sub:`23`\ +C\ :sub:`31`))/9

G\ :sub:`V` = ((C:sub:`11`\ +C\ :sub:`22`\ +C\ :sub:`33`) −
(C:sub:`12`\ +C\ :sub:`23` + C\ :sub:`31`) +3
(C:sub:`44`\ +C\ :sub:`55`\ +C\ :sub:`66`))/15

K\ :sub:`R` = ((s:sub:`11`\ +s\ :sub:`22`\ +s\ :sub:`33`) +
2(s\ :sub:`12`\ +s\ :sub:`23`\ +s\ :sub:`31`))\ :sup:`-1`

G\ :sub:`R` = 15(4(s\ :sub:`11`\ +s\ :sub:`22`\ +s\ :sub:`33`) -
4(s\ :sub:`12`\ +s\ :sub:`23`\ +s\ :sub:`31`) +
3(s\ :sub:`44`\ +s\ :sub:`55`\ +s\ :sub:`66`))\ :sup:`-1`

K\ :sub:`VRH` =(K\ :sub:`V`\ +K\ :sub:`R`)/2

G\ :sub:`VRH` =(G\ :sub:`V`\ +G\ :sub:`R`)/2

ν = (3K:sub:`VRH` − 2G\ :sub:`VRH`)/(6K\ :sub:`VRH`\ +2G\ :sub:`VRH`))

Here K\ :sub:`V` and G\ :sub:`V` are Voigt bulk and shear modulus, and
K\ :sub:`R` and G\ :sub:`R` Reuss-bulk and shear modulus respectively.
The homogenous Poisson ratio is calculated as ν. The EC data can be also
used to predict the ductile and brittle nature of materials with Pugh
(Gv/Kv) and Pettifor criteria (C:sub:`12`-C:sub:`44`) . Materials with
Pugh ratio value >0.571 and Pettifor criteria <0 should be brittle,
while materials with Pugh ratio value <0.571 and Pettifor criteria >0
should be ductile.
...
step by multiplying the *C\ ij* components :math:`(i,j \neq 3)` by the
length of the vacuum padding. Therefore, the units of EC turn into
Nm\ :sup:`-1` from Nm\ :sup:`-2`. For example, in order to calculate
C\ :sub:`11` (stress computed in x direction), the area is computed
using normal of y and z-vectors. Obviously, the z-vector is arbitrary,
so if we multiply the output by z-vector magnitude we get rid of the
arbitrariness of z and also get C\ :sub:`11` in Nm\ :sup:`-1`. As shown
in Fig. 1, the z-vector magnitude is the z-simulation. The above
discussion can also be expressed as the following:

:math:`\sigma_{\text{VASP}} = \frac{F}{A} = \left. \ \frac{F}{\left| z \right|\left| l \right|} \right|_{l \in \left( x,y \right)}`

:math:`\sigma_{\text{mono}} = {\left| z \right| \times \ \sigma}_{\text{VASP}}`
...
TiO\ :sub:`2`-anatase 314    196   191.9  **GaAs**      1174   62    75.6
TiO\ :sub:`2`-rutile  10036  226.3 243.5  **P (black)** 7818   41    36
MAE (GPa):            8.51                                          
===================== ====== ===== ===== ============= ====== =====
=====

|C:\Users\knc6\Desktop\Work\JARVIS\JARVIS-DFT-20160822T155347Z\JARVIS-DFT\JARVIS-ELAST\kvbokeh_plot.png|
...
Thermoelectrics\ `1-4 <#_ENREF_1>`__ are materials that can convert a
temperature gradient into electric voltage, or vice-versa.
Themoelectrics can be used to regenerate electricity from waste
heat\ `5 <#_ENREF_5>`__, refrigeration\ `6 <#_ENREF_6>`__ and several
other space-technology
applications\ `7 <#_ENREF_7>`__\ :sup:`\ `\ `8 <#_ENREF_8>`__\ :sup:`,`.
The search for efficient thermoelectric materials is an area of intense
research due the potential of converting waste heat into electrical
power, and therefore improving energy efficiency and reducing fossil
fuel usage. The figure of merit for thermoelectric materials is the
dimensionless quantity *zT*:

:math:`zT = \frac{S^{2}\sigma}{k_{e} + k_{l}}T`

where *S*, :math:`\sigma`, *k\ e, k\ l*, and *T* are the Seebeck
coefficient, electrical conductivity, electronic part of thermal
conductivity, lattice thermal conductivity, and temperature,
respectively. The numerator, :math:`S^{2}\sigma`, is referred to as the
power-factor. To achieve a high *zT*, a material should have a
...
============================== ====== ======== =========== =========
==== === ====
Bi\ :sub:`2`\ Te\ :sub:`3`     116    124.7357 JVASP-25    7.78E+19  p    420 166
Bi\ :sub:`2`\ Se\ :sub:`3`     -70    -136.9   JVASP-1067  -2.20E+19 n    420 166
CuInTe\ :sub:`2`               254    203.5364 JVASP-3495  1.60E+19  P    300 122
CuGaTe\ :sub:`2`               380    448.9839 JVASP-2295  1.00E+18  p    300 122
AgTlTe                         550    721      JVASP-9744  1.00E+17  p    320 62
ErNiSb                         258    268.71   JVASP-1903  1.42E+19  p    335 216
Cu\ :sub:`2`\ ZnSnSe\ :sub:`4` -26.02 -23.98   JVASP-17430 1.00E+18  p    293 121
CoNbSn                         -69    -2.22    JVASP-18668 5.97E+16  p    318 216
AlFe\ :sub:`2`\ V              -107   -32.3911 JVASP-15637 5.00E+20  n    300 225
...
Cu\ :sub:`2`\ Se               258    148.2624 JVASP-18192 2.00E+21  p    900 216
Mg\ :sub:`2`\ Sn               -71.5  -91.3387 JVASP-14507 -2.00E+19 n    400 225
============================== ====== ======== =========== =========
==== === ====
...
materials (bandgap>0.05 eV, n-type and p-type PF>1000 (μW/(mK)2) at 600
K and 10\ 20 cm\ -3 doping concentration). a) n-type Seebeck coefficient
distribution, b) n-type power factor of materials, c) n-type electrical
conducitivity plotted against the absolute values of Seebeck-coefficient
with color-coded power-factor and size of the dots proportional to
bandgaps, d) p-type Seebeck coefficient distribution, e) p-type power
factor of materials, f) p-type power factor plotted against the
bandgaps, g) predicted dimensionality distribution of screened
...
n-type power-factor (>1000 (μW/(mK)2)). Then the probability of finding
the element in a high power-factor material was calculated.*
...
experiments for graphene\ :sup:`16`, 2H-MoS\ :sub:`2`\ :sup:`17`,
2H-NbSe\ :sub:`2`\ :sup:`18`, 2H-WSe\ :sub:`2`\ :sup:`19`,
1T’-WTe\ :sub:`2`\ :sup:`20`, FeSe\ :sup:`21`, black-P\ :sup:`22,23`,
SnSe\ :sup:`24`, Bismuth surface\ :sup:`57,58`. *The JVASP identifiers
can be used to visualize the detailed webpage for each material.*
...
spectral lines are the quadrupole coupling constant ν\ :sub:`Q` =
:math:`\text{eQ}V_{\text{zz}}/h\ `\ and the asymmetry parameter

   :math:`\eta = (V_{\text{xx}} - V_{\text{yy}})/V_{\text{zz}}`,
...
chosen so that
:math:`\left| V_{\text{zz}} \right| \geq \left| V_{\text{yy}} \right| \geq |V_{\text{xx}}|`,
which forces :math:`0 \leq \eta \leq 1`. Note that if the point group of
the site in question is cubic, then by symmetry all components are zero;
if it is tetragonal or hexagonal, then *η* = 0, but *V*\ :sub:`zz` ≠ 0.
...
and pharmaceutical\ :sup:`4-6` to detecting prohibited substances and
...
Gradient, V\ zz (1021\ Vm\ -2) data. The MAD (Mean Absolute Deviation),
...
Cl\ :sub:`2`     855     Cl       55.18\ :sup:`58`      52.85                   54.23\ :sup:`43`            2.33      4.22
Br\ :sub:`2`     840     Br       95.69\ :sup:`58`      88.86                   94.44\ :sup:`43`            6.83      7.14
I\ :sub:`2`      895     I        113.00\ :sup:`58`     108.70                  119.01\ :sup:`43`           4.30      3.81
Be               25056   Be       0.044\ :sup:`59`      0.072                   0.06\ :sup:`60`             0.028     63.64
Mg               14840   Mg       0.048\ :sup:`59`      0.079                   0.04\ :sup:`60`             0.031     64.58
Sc               996     Sc       0.38\ :sup:`59`       1.78                    0.96\ :sup:`60`             1.40      368.4
Ti               14815   Ti       1.61\ :sup:`59,61`    1.64                    1.75\ :sup:`60`             0.03      1.86
Co               858     Co       2.9\ :sup:`59`        0.52                    0.29\ :sup:`60`             2.38      82.06
Zn               1056    Zn       3.48\ :sup:`59`       5.62                    4.29\ :sup:`60`             2.14      61.50
Zr               14612   Zr       4.40\ :sup:`59`       3.50                    4.14\ :sup:`60`             0.90      20.45
Tc               1020    Tc       1.83\ :sup:`59`       1.67                    1.74\ :sup:`60`             0.16      8.74
Ru               987     Ru       0.97\ :sup:`59`       1.52                    1.62\ :sup:`60`             0.55      56.70
Cd               14832   Cd       6.50\ :sup:`59`       7.56                    8.13\ :sup:`60`             1.06      16.31
La               910     La       1.62\ :sup:`59`       2.24                    0.91\ :sup:`60`             0.62      38.27
Hf               14590   Hf       7.33\ :sup:`59`       8.87                    8.12\ :sup:`60`             1.54      21.01
Re               981     Re       5.12\ :sup:`59`       6.14                    6.49\ :sup:`60`             1.02      19.92
Os               952     Os       4.16\ :sup:`59`       6.00                    7.02\ :sup:`60`             1.84      44.23
BI\ :sub:`3`     3630    I        71.29\ :sup:`62`      68.98                   -                           2.31      3.24
CF\ :sub:`3`\ I  32512   I        124.34\ :sup:`63`     123.22                  -                           1.12      0.90
CIN              5758    I        157.21\ :sup:`64`     151.0                   -                           6.21      3.95
NaNO\ :sub:`2`   1429    Na       0.438\ :sup:`65`      0.552                   0.575\ :sup:`66`            0.114     26.03
NaNO\ :sub:`2`   1429    N        11.10\ :sup:`65`      12.194                  11.772\ :sup:`66`           1.094     9.86
Cu\ :sub:`2`\ O  1216    Cu       9.80\ :sup:`67`       6.47                    6.765\ :sup:`66`            3.33      33.98
TiO\ :sub:`2`    10036   Ti       2.21\ :sup:`68`       2.098                   2.269\ :sup:`66`            0.112     5.07
TiO\ :sub:`2`    10036   O        2.38\ :sup:`68`       2.21                    2.235\ :sup:`66`            0.17      7.14
SrTiO\ :sub:`3`  8082    O        1.62\ :sup:`69`       1.24                    1.00\ :sup:`70`             0.38      23.46
BaTiO\ :sub:`3`  8029    O        2.46\ :sup:`69`       3.56                    2.35\ :sup:`70`             1.10      44.72
Li\ :sub:`3`\ N  1375    N        1.04\ :sup:`71`       1.25                    1.09\ :sup:`66`             0.21      20.19
Li\ :sub:`3`\ N  1375    Li(2c)   0.30\ :sup:`71`       0.225                   0.291\ :sup:`66`            0.075     25.00
Li\ :sub:`3`\ N  1375    Li(1b)   0.60\ :sup:`71`       0.50                    0.616\ :sup:`66`            0.144     24.00
FeSi             8178    Fe       4.45\ :sup:`72,73`    4.84                    4.92\ :sup:`43`             0.39      8.76
FeS\ :sub:`2`    2142    Fe       3.0\ :sup:`74`        2.93                    3.21\ :sup:`43`             0.07      2.33
                                                                                                                     
(marcasite)                                                                                                          
FeS\ :sub:`2`    9117    Fe       3.66\ :sup:`74`       3.51                    3.40\ :sup:`43`             0.15      4.10
                                                                                                                     
(pyrite)                                                                                                             
2H-MoS\ :sub:`2` 54      Mo       7.09\ :sup:`39,40`    7.70                    -                           0.61      8.60
2H-MoS\ :sub:`2` 54      S        5.54\ :sup:`39,40`    5.33                    -                           0.21      3.80
2H-WS\ :sub:`2`  72      S        4.82\ :sup:`39,40`    4.53                    -                           0.29      6.22
CaGa\ :sub:`2`   16464   Ga       4.44\ :sup:`75`       3.55                    3.77\ :sup:`76`             0.89      20.05
SrGa\ :sub:`2`   14853   Ga       5.22\ :sup:`75`       2.54                    4.13\ :sup:`76`             2.68      51.34
BaGa\ :sub:`2`   19628   Ga       4.48\ :sup:`75`       5.10                    4.38\ :sup:`76`             0.62      13.84
NaGa\ :sub:`4`   14728   Ga(e)    6.49\ :sup:`76`       5.20                    6.18\ :sup:`76`             1.29      19.88
NaGa\ :sub:`4`   14728   Ga(d)    4.64\ :sup:`76`       4.33                    4.44\ :sup:`76`             0.31      6.68
CaGa\ :sub:`4`   20533   Ga(e)    2.89\ :sup:`76`       2.67                    2.80\ :sup:`76`             0.22      7.61
CaGa\ :sub:`4`   20533   Ga(d)    4.87\ :sup:`76`       4.99                    4.73\ :sup:`76`             0.12      2.46
SrGa\ :sub:`4`   20206   Ga(e)    2.51\ :sup:`76`       1.67                    2.24\ :sup:`76`             0.84      33.47
SrGa\ :sub:`4`   20206   Ga(d)    5.95\ :sup:`76`       5.31                    5.64\ :sup:`76`             0.64      10.76
TaP              79643   Ta       3.00\ :sup:`77`       2.5                     3.54\ :sup:`77`             0.50      16.67
UAs\ :sub:`2`    19797   U        15.0\ :sup:`78`       9.7                     13.03\ :sup:`57`            5.3       35.3
**MAD**                                                                                                     1.17      -
**MAPD**                                                                                                    -         28.90
================ ======= ======== =====================
======================= =========================== ========= ==========
...
shown as a function of V\ :sub:`ZZ` and η. The logarithmic z-scale
allows for the strong preponderance of small V\ :sub:`ZZ` and small η
values to be clearly seen, while at the same time revealing the
bell-shaped distribution in V\ :sub:`ZZ` for all η values. Values where
V\ :sub:`zz`\ =0, and therefore η is ill-defined, have been excluded.
...
Electronic (𝜀:sub:`11`) OPT                                       28         3.2   0 to 60
Electronic (𝜀:sub:`11`) MBJ                                       28         2.62  0 to 60
Solar-eff. (SLME) (%) (MBJ)                                       5          6.55  0 to 33.7
Max. piezoelectric strain coeff (Cm:sup:`-2`)                     16         0.21  0 to 2
Dielectric constant (𝜀:sub:`11`) (DFPT)                           16         2.46  0 to 60
Seebeck coefficient (μV/K)                                        14         54.7  -600 to 600
Electric field gradient V\ :sub:`zz` (10:sup:`21`\ Vm\ :sup:`-2`) 37         1.17  0 to 100
IR mode (cm:sup:`-1`)                                             8          8.36  0 to 4000
=================================================================
========== ===== =============
...
..
   ...
   downloaded all the corresponding crystal structures from the
   Materials Project (MP) or JARVIS-DFT database. We also downloaded all
   the energetics and mechanical properties data from and stored them in
   a separate database for a later comparison with the classical results
   (LAMMPS calculations at T=0 K). The high-throughput setting of LAMMPS
   jobs was done using JARVIS-tools. In our runs we used 10-06 as
   strain, 10-10 eV/Å for force convergence during the minimization to
   optimize the structure and 1000 maximum iteration for structure
   optimization. These are generalized computational set-up parameters,
   and the energetics and elastic constant data may or may not depend on
   them. We tested strain parameters for a range of values
   (10:sup:`-04`, 10\ :sup:`-06` and 10\ :sup:`-08`) but obviously
   evaluating such set of parameters for all the calculations was too
   extensive a work and was not carried out here. The relaxed structure
   was also stored along with the above files for later use such as for
   performing defect, phonon or other similar calculations.
   ...
   positions were identified and deleted to represent
   vacancy-structures. The multiplicity of the Wyckoff positions is also
   recorded. After the defect structure generation, the LAMMPS energy
   minimization is carried out. In a subsequent run, we calculate the
   chemical potential of the defect element using the specific
   force-field.
   ...
   energies. The most stable prototype for chemical potential
   calculation was determined using the energy above convex hull data
   from DFT. The defect structures were required to be at least 1.5 nm
   long in the x, y and z directions to avoid self-interactions with the
   periodic images of the simulation cell. Similar to the defect
   structures, distinct surfaces were created up to 3 Miller indices
   with the relaxed structure stored in the JSON file. A generic code
   for generating defect and surface structures is given at our github
   page. We enforce the surfaces to be at least 2.5 nm thick and with
   2.5 nm vacuum in the simulation box. The 2.5 nm vacuum is used to
   ensure no self-interaction and the thickness is used to mimic actual
   ...
   Phonons were obtained by making an interface of JARVIS-FF with
   Phonopy package at 0 K. For deformed-structures, constant volume
   ensemble was used. The deformed structures were taken of at least 1.5
   nm size in all directions.
...
IR mode (OPT) (cm:sup:`-1`)          3411          77.84 316.7
...
.. |JARVIS-DFT overview| image:: media/image1.png
   :width: 6.5in
   :height: 3.20833in
.. |image1| image:: media/image2.png
   :width: 3.96181in
   :height: 3.72339in
.. |image2| image:: media/image22.wmf
   :width: 0.22917in
   :height: 0.26042in
.. |image3| image:: media/image23.wmf
   :width: 0.19792in
   :height: 0.26042in
.. |image4| image:: media/image26.png
   :width: 4.84028in
   :height: 5.70388in
.. |image5| image:: media/image27.png
   :width: 6.5in
   :height: 2.16875in
.. |image6| image:: media/image28.png
   :width: 6.5in
   :height: 1.99236in
.. |image7| image:: media/image29.png
   :width: 6.5in
   :height: 1.56319in
.. |C:\Users\knc6\Downloads\Ef.png| image:: media/image31.png
   :width: 5.23611in
   :height: 3.92708in
.. |image9| image:: media/image32.png
   :width: 6.5in
   :height: 1.16875in
.. |image10| image:: media/image33.png
   :width: 6.5in
   :height: 1.875in
...
.. |image15| image:: media/image38.png
   :width: 2.68981in
   :height: 3.44204in
.. |image16| image:: media/image39.png
   :width: 6.5in
   :height: 3.61591in
.. |image17| image:: media/image40.png
   :width: 6.5in
   :height: 3.52986in
.. |C:\Users\knc6\Downloads\mbj_op3.png| image:: media/image41.png
   :width: 6.5in
   :height: 4.875in
.. |image19| image:: media/image42.png
   :width: 6.5in
   :height: 2.07361in
.. |image20| image:: media/image43.png
   :width: 4.28472in
   :height: 0.91462in
.. |C:\Users\kamal\AppData\Local\Microsoft\Windows\INetCache\Content.MSO\9A11865B.tmp| image:: media/image44.png
   :width: 6.5in
   :height: 5.41111in
.. |image22| image:: media/image45.png
   :width: 6.5in
   :height: 3.70833in
...
.. |image27| image:: media/image50.png
   :width: 4.22222in
   :height: 2.82745in
.. |C:\Users\kamal\AppData\Local\Microsoft\Windows\INetCache\Content.MSO\1F32B701.tmp| image:: media/image51.png
   :width: 6.5in
   :height: 6.5in
.. |image29| image:: media/image52.png
   :width: 6.5in
   :height: 1.76806in
.. |C:\Users\knc6\Downloads\Allcij.png| image:: media/image53.png
   :width: 3.57666in
   :height: 2.68056in
.. |C:\Users\knc6\Desktop\Work\JARVIS\JARVIS-DFT-20160822T155347Z\JARVIS-DFT\JARVIS-ELAST\kvbokeh_plot.png| image:: media/image54.png
   :width: 6.5in
   :height: 3.71505in
...
.. |image33| image:: media/image56.png
   :width: 4.52778in
   :height: 4.19158in
.. |image34| image:: media/image57.png
   :width: 6.5in
   :height: 4.00347in
.. |image35| image:: media/image58.png
   :width: 4.3125in
   :height: 0.9869in
.. |C:\Users\kamal\AppData\Local\Microsoft\Windows\INetCache\Content.MSO\39BDF3E3.tmp| image:: media/image59.png
   :width: 6.5in
   :height: 4.01111in
.. |image37| image:: media/image60.png
   :width: 6.5in
   :height: 3.71458in
.. |image38| image:: media/image61.png
   :width: 6.5in
   :height: 2.61111in
.. |image39| image:: media/image62.png
   :width: 6.5in
   :height: 4.83333in
.. |image40| image:: media/image63.png
   :width: 6.5in
   :height: 2.59861in
.. |image41| image:: media/image64.png
   :width: 6.5in
   :height: 4.34306in
.. |image42| image:: media/image65.png
   :width: 6.5in
   :height: 1.01597in
.. |image43| image:: media/image66.png
   :width: 6.02049in
   :height: 4.70254in
.. |image44| image:: media/image67.png
   :width: 6.5in
   :height: 2.3875in
.. |C:\Users\knc6\Desktop\Work\JARVIS\JARVIS-DFT-20160822T155347Z\JARVIS-DFT\JARVIS-CONVRG\NewFigs\Fig2.jpg| image:: media/image68.jpeg
   :width: 6.5in
   :height: 4.64339in
.. |Screen shot of JARVIS-FF| image:: media/image69.png
   :width: 6.5in
   :height: 3.41944in
.. |image47| image:: media/image70.png
   :width: 3.25209in
   :height: 4.21487in
.. |C:\Users\Kamal\AppData\Local\Microsoft\Windows\INetCacheContent.Word\Fig.3.tif| image:: media/image71.tiff
   :width: 6.63796in
   :height: 3.73385in
.. |C:\Users\knc6\Desktop\Work\JARVIS\Bv.png| image:: media/image72.png
   :width: 5.08667in
   :height: 3.63333in
.. |C:\Users\knc6\Downloads\VacSurfMPJF.png| image:: media/image73.png
   :width: 6.4992in
   :height: 3.26772in
.. |C:\Users\knc6\Desktop\Work\JARVIS\JARVIS-ML\Figure-1.png| image:: media/image74.png
   :width: 6.5in
   :height: 3.65676in
.. |C:\Users\knc6\Downloads\form-all.png| image:: media/image75.png
   :width: 6.5in
   :height: 4.33333in
